...
// graph-first / LLM-second: the model reasons over this proven output; it never invents edges.
...
export function gate({ blast, cascade, change, blastEdges = [] }) {
  ...
  // Confidence: lower when edges come from high tiers or are LLM-inferred.
  let confidence = 'high';
  ...
  // which is byte-for-byte the answer given for a node that is indexed and genuinely has
  // no dependents. The first means "not looked at"; the second means "safe to change".
  // Reporting PASS for both is this gate answering a question it was never asked.
  if (blast && blast.seedKnown === false) {
    return {
      verdict: 'WARN',
      confidence: 'none',
      summary: `WARN · seed not in graph · cascade=${c} change=${change} confidence=none`,
      top: [],
      reasons: [
        'the node is NOT in the graph: its blast radius was never measured, only unfound',
        'an empty result here is the absence of a lookup, not the absence of dependents',
        'index the file, or check the identifier matches one the graph holds, then re-run',
      ],
      honest:
        'No reachability was computed: the seed is absent from the graph. This is not a low-risk verdict, it is no verdict.',
    };
  }
