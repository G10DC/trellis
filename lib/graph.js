...
/** Shared BFS core over a prebuilt adjacency map. Returns { seen, frontier } where
 *  frontier is the last unexpanded layer (non-empty with unvisited neighbours => depth truncated). */
function bfs(adj, seed, { depth = 5, reverse = false, types = null } = {}) {
 ...
 */
export function reachability(g, seed, opts = {}) {
  const { out, in: inn } = adjacency(g);
  const seen = bfs(opts.reverse ? inn : out, seed, opts).seen;
  // Whether the seed is in the graph at all.
  //
  // A seed that was never indexed traverses nothing and comes back with an empty set —
  // identical to a node the graph knows about and nothing depends on. Those are opposite
  // answers: one says "safe to change", the other says "not looked at". Marking the
  // result is what lets gate() tell them apart; the value is still a Set, so nothing
  // that iterates it changes.
  seen.seedKnown = g.nodes.has(seed);
  return seen;
}
 ...
 */
export function reachabilityBounded(g, seed, opts = {}) {
  const { out, in: inn } = adjacency(g);
  const adj = opts.reverse ? inn : out;
  const { seen, frontier } = bfs(adj, seed, opts);
  const typeSet = opts.types ? new Set(Array.isArray(opts.types) ? opts.types : [opts.types]) : null;
  let truncated = false;
  for (const n of frontier) {
    for (const e of adj.get(n) ?? []) {
      if (typeSet && !typeSet.has(e.type)) continue;
      const m = opts.reverse ? e.from : e.to;
      if (!seen.has(m)) { truncated = true; break; }
    }
    if (truncated) break;
  }
  return { reached: seen, truncated, depth: opts.depth ?? 5 };
}
