// lib/model-edges.js — merge tier-3 processing engine-inferred edges into a graph (Phase 3).
// The processing engine returns JSON edges (see templates/tier3-extract.md); this module validates and merges them.
 ...
 * @param {object[]} edges  processing engine-returned edges
...
export function applyLLMEdges(graph, edges) {
  ...
  for (const e of edges) {
    ...
    // ensure referenced nodes exist (processing engine may surface implicit ones the deterministic tiers missed)
    ...
    addEdge(graph, {
      ...
      type: e.type,
      tier: 3,
      inferred: e.inferred !== false, // default true; processing engine may mark false only for statically-grounded-but-tier3 edges
      evidence: e.evidence,
    });
    report.accepted++;
  }
