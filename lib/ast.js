...
//
// tree-sitter is the planned multi-language backend; acorn is the pure-JS AST for the
// JS/TS family, chosen to keep zero native deps (see refs/extractors.md).

import { parse } from 'acorn';
import { createGraph, addNode, addEdge } from './graph.js';
import { resolveSpec, bareTop, resolveFile } from './paths.js';
...
export function buildSymbolGraph(modules, manifest = {}) {
  ...
  // imports: file->file (extension-aware) or ->external, + imported-name index per file
  const fileSet = new Set(modules.map((m) => m.file));
  const importedByFile = new Map();
  for (const mod of modules) {
    const im = new Map();
    for (const i of mod.imports) {
      const baseTarget = resolveSpec(mod.file, i.spec);
      const target = baseTarget ? resolveFile(baseTarget, fileSet) : null;
      if (target) addEdge(g, { from: mod.file, to: target, type: 'imports', tier: 2, inferred: false, evidence: `${mod.file}:${i.line}` });
      else if (!baseTarget) {
        const extId = `external:${bareTop(i.spec)}`;
        if (!g.nodes.has(extId)) addNode(g, { id: extId, kind: 'external', file: bareTop(i.spec), line: 0, exported: false, labels: ['external'] });
        addEdge(g, { from: mod.file, to: extId, type: 'imports', tier: 1, inferred: false, evidence: `${mod.file}:${i.line}` });
      }
      // relative spec with no matching file: skip (avoids a dangling edge)
