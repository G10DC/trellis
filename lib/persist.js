 ...
 *
 * Reports how much was actually inspected. `ok: true` over an empty graph is a
 * statement about an empty graph, not about a healthy one -- and an empty graph
 * is what you get from a path that does not exist, because `DatabaseSync` happily
 * creates it and `ensureSchema` fills in empty tables. A caller pointed at the
 * wrong file used to receive a clean bill of health for a database it never read.
 *
 * `checked` is the number of edges examined and `measured` is false when there
 * were none. `ok` keeps its meaning (no dangling edges found) for existing
 * callers; `measured` is the field that separates "nothing wrong" from "nothing".
 */
export function integrityCheck(dbPath) {
  const existeva = dbPath !== ':memory:' && fs.existsSync(dbPath);
  const db = new DatabaseSync(dbPath);
  try {
    ensureSchema(db);
    ...
    const checked = db.prepare('SELECT COUNT(*) AS n FROM edges').get().n;
    return {
      ...
      existed: existeva,
      honest: checked > 0
        ? `${checked} edges checked, ${dangling.length} dangling.`
        : (existeva
            ? 'The database exists but holds no edges: nothing was checked.'
            : `No database at ${dbPath}: an empty one was created, so this result describes nothing.`),
    };
  } finally {
    db.close();
  }
