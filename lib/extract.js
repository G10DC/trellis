...
// tier 2: tree-sitter AST — precise calls/references/inheritance.   DEFERRED (Phase 2). see refs/extractors.md
// tier 3: LLM — implicit deps (DI, events, reflection, dynamic dispatch). DEFERRED (Phase 3). see templates/tier3-extract.md
//
...
function extractPython(file, source) {
  ...
  let m;
  RE_PY.def.lastIndex = 0;
  while ((m = RE_PY.def.exec(source))) addDef(m[1], lineOf(source, m.index));
  RE_PY.cls.lastIndex = 0;
  while ((m = RE_PY.cls.exec(source))) addDef(m[1], lineOf(source, m.index));
  RE_PY.importFrom.lastIndex = 0;
  while ((m = RE_PY.importFrom.exec(source))) {
    const spec = m[1];
  ...
  RE_PY.importBare.lastIndex = 0;
  while ((m = RE_PY.importBare.exec(source))) {
    const line = lineOf(source, m.index);
...
export function extractFile(file, source, { lang } = {}) {
  ...
  let m;
  RE.def.lastIndex = 0;
  while ((m = RE.def.exec(source))) {
    const name = m[1];
    const idx = m.index;
    const exported = /^export\b/.test(source.slice(idx, idx + 8));
    nodes.push({
      id: nodeId(file, name),
      kind: 'symbol',
      file,
    ...
    edges.push({ from: file, to: nodeId(file, name), type: 'defines', tier: 0, inferred: false, evidence: `${file}:${lineOf(source, idx)}` });
  }

  RE.import.lastIndex = 0;
  while ((m = RE.import.exec(source))) {
    const spec = m[4] || m[5];
    const line = lineOf(source, m.index);
    const target = resolveSpec(file, spec);
    if (!target) {
      unresolved.push({ file, spec, line });
      continue;
    }
