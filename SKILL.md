...
name: trellis
status: implemented
description: Build a lightweight dependency graph (tier-0 imports, tier-1 manifests, tier-2 acorn AST) and consult its bidirectional reachability BEFORE editing, so no dependent is missed. Returns a PASS/WARN/BLOCK verdict with confidence, never claiming reachability equals breakage. Graph-first, LLM-second. Use before non-trivial edits on medium/large JS/TS codebases touching a shared symbol. Never treat a PASS as proof nothing will break.
---
...
3. **Graph-first, LLM-second.** The model reasons over edges the extractor *proved*; it never invents edges or impact sets.
4. **Read the confidence.** tier-0/1 edges are syntactic; tier-3 edges are LLM-inferred and lower trust. Absence of edges is not absence of dependents.
...
7. **Snapshot for hygiene, not sharing.** `trellis index --out` persists a graph snapshot to `<root>/.trellis/graph.jsonl`, consumed by trellis's own `changed` command to detect dangling edges after an edit. No other skill in this repo reads that file — `.ast-cache.json` does not exist; earlier docs claiming `smith`/`prism-search` consume a shared AST cache were wrong.
...
- `index <root> [--out]` — build + stats; with `--out`, persist a graph snapshot to `<root>/.trellis/graph.jsonl`, consumed by `changed` (below). Not consumed by any other skill.
...
`0` regex (imports) · `1` manifest (externals) · `2` acorn/AST — calls/refs/inheritance, cross-file ✅ · `3` LLM-inferred (DI/events/reflection) ✅ via `lib/model-edges.js` · `4` MCP-resolved (optional). Edges carry `tier` + `inferred`; the gate (`lib/gate.js`) + `lib/refine.js` lower confidence when inferred edges are in the blast.
...
## Status
Phases 0–5 complete (71 offline tests): tier-0/1/2 extraction (regex + manifest + acorn AST, JS/TS + Python), in-memory graph + SQLite persistence (`WITH RECURSIVE`), bidirectional reachability with depth/type filters, SCC condensation, bounded closure, memoized closure, PASS/WARN/BLOCK gate with confidence + change-type, semantic refinement, tier-3 LLM-edge merge, incremental sync via `git diff`, integrity check + rebuild, precision-study harness, PR-level impact. Graph-first/LLM-second; honest (reachability ≠ breakage).

## When to use

- Before editing a shared or exported symbol on a medium/large JS/TS (or Python, tier-0/1 only)
